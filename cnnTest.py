...
class WatermarkGeneration(nn.Module):
    ...
    def masking(self, x, key, m=8, n=8):
        ...
        x.index_put_((torch.LongTensor(a) , torch.LongTensor(b) , torch.LongTensor(c) ,
                      torch.LongTensor(d) ),
                     torch.Tensor(e) )
...
watermarkGen = torch.load('./pkl/watermarkGen.pkl',map_location=torch.device('cpu'))
tExtN = torch.load('./pkl/tExtN.pkl',map_location=torch.device('cpu'))
fExtN = torch.load('./pkl/fExtN.pkl',map_location=torch.device('cpu'))
tMatN = torch.load('./pkl/tMatN.pkl',map_location=torch.device('cpu'))
...
for step, (data, y) in enumerate(trainData):
    ...
    print('batch {}:'.format(str(step)))
    print('real:')
    print(RST)
    print('generated:')
    print(out.detach().numpy()[0][0])
    print('batch avg loss:', L.item())
    print('')
